...
- **Recommended**: at least **3** replicas per cell. The `ANY_2` durability policy requires 1 primary + 2 synchronous standbys so that one standby can be drained during rolling upgrades while the other maintains write quorum. The operator issues an admission warning if a pool has fewer than 3 replicas.
