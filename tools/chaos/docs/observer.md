# Observer Reference (Level 0)

The observer is the foundation of the chaos tool. It runs a continuous loop (default interval: 10 seconds) performing 9 categories of health checks against a Multigres cluster. Every finding is emitted as a structured JSON log line and recorded as a Prometheus metric.
...
- **Writes:** Nothing
- **Latency:** Measured and recorded to `multigres_chaos_probe_latency_seconds{check="sql-probe"}`
...
```sql
BEGIN;
CREATE TEMP TABLE IF NOT EXISTS _chaos_write_probe (x int);
ROLLBACK;
```

- **Purpose:** Detect whether writes are blocked on the primary
- **Reads:** Nothing
- **Writes:** Creates a temporary table inside a transaction that is **always rolled back**. The temp table `_chaos_write_probe` is session-scoped and would be dropped when the connection closes anyway, but the explicit `ROLLBACK` ensures nothing persists even transiently.
...
```bash
# Just fatals
kubectl logs -l app.kubernetes.io/name=multigres-chaos -n multigres-operator | grep '"fatal"'

# Just replication issues
kubectl logs -l app.kubernetes.io/name=multigres-chaos -n multigres-operator | grep '"replication"'

# Cycle summaries only
kubectl logs -l app.kubernetes.io/name=multigres-chaos -n multigres-operator | grep 'observer cycle complete'

# Split-brain
kubectl logs -l app.kubernetes.io/name=multigres-chaos -n multigres-operator | grep 'SPLIT-BRAIN'
```
