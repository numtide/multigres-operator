...
import (
	...
	"github.com/numtide/multigres-operator/tools/chaos/pkg/observer"
...
func main() {
	...
	flag.IntVar(&level, "level", envInt("CHAOS_LEVEL", 0), "Chaos level (0=observer, 1=exerciser, 2=fault, 3=adversarial)")
	...
	logger.Info("multigres-chaos starting",
		"version", version,
		"commit", gitCommit,
		"buildDate", buildDate,
		"level", level,
	...
	if level != 0 {
		logger.Error("only level 0 (observer) is implemented", "requestedLevel", level)
		os.Exit(1)
	}
